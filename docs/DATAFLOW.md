...
| **RabbitMQ** | 訊息佇列(任務排隊區) | `rabbitmq-network.yml` (Docker 部署) |
...
producer
   ...
   └──── 2. send_task ───────►  RabbitMQ (任務佇列)
                              ...
                              Celery Worker (容器)
                                     ...
                                     │
                                     │ 6. 篩 ETF + 排名取 Top 20
                                     │    呼叫 calculate_net_buy()
                                     │
                                     │ 7. 呼叫 fetch_ohlcv_batch()
                                     │    ↓
                                     │    Yahoo (yfinance)
                                     │    (一次批次拉 20 檔股價)
                                     │
                                     ...
                                     │    呼叫 compute_metrics()
                                     │
                                     │ 9. upload_to_mysql_v2()
                                     ▼
                              MySQL (EtfTop20BuyInstitutionalV2)
                                     │
                                     │ 每天 20 筆乾淨資料
```
...
### 方法 A:phpMyAdmin(網頁,最直覺)

打開 `http://localhost:8080`,登入後選 `mydb` → `EtfTop20BuyInstitutionalV2` → Browse。
...
engine = create_engine('mysql+pymysql://root:<你的密碼>@127.0.0.1:3307/mydb')
...
### 方法 C:Docker exec 直查(最快)

```bash
docker exec $(docker ps -q -f name=mysql_mysql) mysql -uroot -p<密碼> -e "SELECT COUNT(*), MAX(date), MIN(date) FROM mydb.EtfTop20BuyInstitutionalV2;"
```
...
完整 schema 定義在 `crawler/sql/etf_top20_v2_schema.sql`。
